...
from triad import POLICtoTriad
...
class StackMachine:
    ...
    def abs(self, item, father=0):
        ...
        if item.name not in Lexer.token and not item.name == 'METHOD':
            ...
            if item.name == 'RFBreaket':
                ...
                if len(father.children) == 4 and not len(self.buf):
    ...
    def start(self):
        for item in self.input:
            ...
            self.stack = []
        print(self.output)
        self.output = POLICtoTriad(self.output)
        print(self.output)
        self.compilation()
